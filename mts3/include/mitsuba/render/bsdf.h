...
enum class BSDFFlags : uint32_t {
    ...
    /// (in our case) Does the bsdf has differentiable parameters
    ///               potential confict with 'YanGlintyBSDF', though this should not be a problem
...
template <typename Float, typename Spectrum> struct BSDFSample3 {
    ...
    /// isotropic roughness at the sample
    Float alpha;
    ...
    BSDFSample3(const Vector3f &wo)
        : wo(wo), pdf(0.f), eta(1.f), alpha(1.f), sampled_type(0),
    ...
    DRJIT_STRUCT(BSDFSample3, wo, pdf, eta, alpha, sampled_type, sampled_component);
};
...
public:
    ...
    virtual Color3f compute_normalmap_deriv(const BSDFContext &ctx,
                                            const SurfaceInteraction3f &si,
        const Vector3f& wo,
                            const Vector3f &d_output, // input parameter
        Mask active = true) const {
        DRJIT_MARK_USED(ctx);
        DRJIT_MARK_USED(si);
        DRJIT_MARK_USED(wo);
        DRJIT_MARK_USED(d_output);
    ...
    virtual UInt32 get_texel_index(const SurfaceInteraction3f &si,
        ...
        DRJIT_MARK_USED(si);
        DRJIT_MARK_USED(wo);
        DRJIT_MARK_USED(texture_id);
        DRJIT_MARK_USED(active);
        return 0u;
    }

    // eval bsdf with a specified (isotropic)roughness
    virtual Spectrum eval_with_roughness(const BSDFContext &ctx,
                                              const SurfaceInteraction3f &si,
                                              const Vector3f &wo,
                                              Float roughness,
                                              Mask active = true) const {
        DRJIT_MARK_USED(roughness);
        return eval(ctx, si, wo, active);
        // for this new method, by default we simply fall back to eval
    }
...
DRJIT_VCALL_TEMPLATE_BEGIN(mitsuba::BSDF)
    ...
    DRJIT_VCALL_METHOD(eval_with_roughness)
    ...
    DRJIT_VCALL_METHOD(compute_normalmap_deriv)
