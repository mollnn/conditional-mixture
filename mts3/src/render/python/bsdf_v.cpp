...
MI_PY_EXPORT(BSDFSample) {
    ...
    auto bs = py::class_<BSDFSample3f>(m, "BSDFSample3f", D(BSDFSample3))
        ...
        .def_readwrite("alpha", &BSDFSample3f::alpha, D(BSDFSample3, alpha))
    ...
    MI_PY_DRJIT_STRUCT(bs, BSDFSample3f, wo, pdf, eta, alpha, sampled_type, sampled_component);
}
...
public:
    ...
    Color3f compute_normalmap_deriv(const BSDFContext &ctx,
                                    ...
                                    Mask active) const override {
        PYBIND11_OVERRIDE(Color3f, BSDF, compute_normalmap_deriv, ctx, si, wo, d_output, active);
    }
...
template <typename Ptr, typename Cls> void bind_bsdf_generic(Cls &cls) {
    ...
    cls.def("sample",
       ...
       .def("eval_attribute_3",
        ...
        .def("eval_with_roughness",
            [](Ptr bsdf, const BSDFContext &ctx, const SurfaceInteraction3f &si,
               const Vector3f &wo, Float roughness,
               Mask active) { return bsdf->eval_with_roughness(ctx, si, wo, roughness, active); },
            "ctx"_a, "si"_a, "wo"_a, "roughness"_a, "active"_a = true, D(BSDF, eval_with_roughness))
        .def("compute_normalmap_deriv",
            [](Ptr bsdf, const BSDFContext &ctx, const SurfaceInteraction3f &si,
               const Vector3f &wo, const Vector3f &d_output, Mask active) {
                return bsdf->compute_normalmap_deriv(ctx, si, wo, d_output, active);
            },
            "ctx"_a, "si"_a, "wo"_a, "d_output"_a, "active"_a = true, D(BSDF, compute_normalmap_deriv))
        .def("get_texel_index",
            [](Ptr bsdf, const SurfaceInteraction3f &si, const Vector3f &wo,
               const std::string &texture_id, Mask active) {
                return bsdf->get_texel_index(si, wo, texture_id, active);
            },
            "si"_a, "wo"_a, "texture_id"_a, "active"_a = true, D(BSDF, get_texel_index));
