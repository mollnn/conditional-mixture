...
public:
    ...
    sample(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        bs.alpha = roughness; // not care about anisotropic now
    ...
    Spectrum eval(const BSDFContext& ctx, const SurfaceInteraction3f& si,
        ...
        //return eval_with_roughness(ctx, si, wo, m_roughness->eval_1(si, active), active);
    ...
    Spectrum eval_with_roughness(const BSDFContext &ctx,
                            ...
                            Float roughness, Mask active) const override {
        MI_MASKED_FUNCTION(ProfilerPhase::BSDFEvaluate, active);
        ...
        Float anisotropic = m_has_anisotropic ? m_anisotropic->eval_1(si, active) : 0.0f,
             // roughness = m_roughness->eval_1(si, active),  
             // roughness comes from parameter now
              flatness = m_has_flatness ? m_flatness->eval_1(si, active) : 0.0f,
              spec_trans = m_has_spec_trans ? m_spec_trans->eval_1(si, active) : 0.0f,
              metallic = m_has_metallic ? m_metallic->eval_1(si, active) : 0.0f,
              clearcoat = m_has_clearcoat ? m_clearcoat->eval_1(si, active) : 0.0f,
    ...
    UInt32 get_texel_index(const SurfaceInteraction3f &si, 
        ...
        const std::vector<ref<Texture>> textures{
            m_base_color, m_roughness,       m_anisotropic, m_sheen,
            m_sheen_tint, m_spec_trans,      m_flatness,    m_spec_tint,
            m_clearcoat,  m_clearcoat_gloss, m_metallic,
        };

        for (unsigned int i = 0; i < textures.size(); ++i) {
            if (textures[i] && textures[i]->id() == texture_id)
