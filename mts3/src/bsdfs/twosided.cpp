...
public:
    ...
    TwoSidedBRDF(const Properties &props) : Base(props) {
        ...
        if (m_brdf[0]->needs_differentials() ||
            m_brdf[1]->needs_differentials()) {
    ...
    Spectrum eval_with_roughness(const BSDFContext &ctx_, const SurfaceInteraction3f &si_,
                  const Vector3f &wo_, Float roughness,
                  Mask active
        ) const override {
        MI_MASKED_FUNCTION(ProfilerPhase::BSDFEvaluate, active);
        ...
        if (m_brdf[0] == m_brdf[1]) {
            wo.z()    = dr::mulsign(wo.z(), si.wi.z());
            si.wi.z() = dr::abs(si.wi.z());
            result    = m_brdf[0]->eval_with_roughness(ctx, si, wo, roughness, active);
        } else {
            Mask front_side = Frame3f::cos_theta(si.wi) > 0.f && active,
                 back_side  = Frame3f::cos_theta(si.wi) < 0.f && active;

            if (dr::any_or<true>(front_side))
                result = m_brdf[0]->eval_with_roughness(ctx, si, wo, roughness, front_side);
            ...
            if (dr::any_or<true>(back_side)) {
                ...
                dr::masked(result, back_side) =
                    m_brdf[1]->eval_with_roughness(ctx, si, wo, roughness, back_side);
            }
        }
    ...
    Color3f compute_normalmap_deriv(const BSDFContext &ctx,
        ...
        //assert(m_brdf[0] == m_brdf[1]); //in our case
        if (m_brdf[0] && dr::any_or<true>(front_side)) {
            return m_brdf[0]->compute_normalmap_deriv(ctx, si, wo, d_output, front_side);
        }

        if (m_brdf[1] && dr::any_or<true>(back_side)) {
            Vector3f wo_(wo);
            SurfaceInteraction3f si_(si);
            wo_.z()    = dr::mulsign(wo_.z(), si_.wi.z());
            si_.wi.z() = dr::abs(si_.wi.z());  
            return m_brdf[1]->compute_normalmap_deriv(ctx, si_, wo_, d_output, back_side);
        }
        return 0.f;
    }
    ...
    UInt32 get_texel_index(const SurfaceInteraction3f &si, 
        ...
        if (m_brdf[0] && dr::any_or<true>(front_side))
            return m_brdf[0]->get_texel_index(si, wo, texture_id, front_side);

        if (m_brdf[1] && dr::any_or<true>(back_side))
            return m_brdf[1]->get_texel_index(si, wo, texture_id, back_side);
