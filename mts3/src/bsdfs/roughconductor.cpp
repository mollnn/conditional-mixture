...
public:
    ...
    std::pair<BSDFSample3f, Spectrum> sample(const BSDFContext &ctx,
        ...
        bs.eta = 1.f;
        bs.alpha = m_alpha_u->eval_1(si, active);
    ...
    Spectrum eval_with_roughness(const BSDFContext &ctx, const SurfaceInteraction3f &si,
                  const Vector3f &wo, Float roughness, Mask active) const override {
        MI_MASKED_FUNCTION(ProfilerPhase::BSDFEvaluate, active);
        ...
        /* Construct a microfacet distribution matching the
           roughness values at the current surface position. */
        MicrofacetDistribution distr(m_type, roughness, roughness, m_sample_visible); // !
