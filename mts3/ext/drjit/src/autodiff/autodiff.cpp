...
struct Variable {
    ...
    T accum(const T& v, uint32_t src_size) {
        T my_ans;
     ...
     */
    template <typename T = Value>
    T mul_accum(const T &v1, const T &v2_, uint32_t src_size) {
        ...
        if constexpr (is_array_v<T>) {
            ...
            if (size == 1 && src_size != 1) {
                ...
                if (grad_valid)
                    ...
                    grad = std::move(v3), my_ans = grad;
            } else {
                if (grad_valid)
                    grad = fmadd(v1, v2, grad), my_ans = v1 * v2;
                else
                    grad = v1 * v2, my_ans = v1 * v2;
            }
        } else {
...
template <typename T> void ad_scope_leave(bool process_postponed) {
    ...
    if (scope.isolate && !scope.postponed.empty()) {
        ...
        if (process_postponed) {
            ...
            Value my_ans = ad_traverse<Value>(ADMode::Backward,
                               (uint32_t) ADFlag::ClearVertices); // !
            ad_trace("ad_scope_leave(): return from ad_traverse  my_ans=%d", my_ans);
        } else {
            scopes.pop_back();
        }
    } else {
...
template <typename Value> struct MaskEdge : Special {
    ...
    Value my_backward(Variable *source, const Variable *target, uint32_t) const {
        return source->accum(!negate ? detail::and_(target->grad, mask)
                              : detail::andnot_(target->grad, mask),
                      target->size);
    }
...
template <typename Value> struct GatherEdge : Special {
    ...
    Value my_backward(Variable *source, const Variable *target, uint32_t) const {
        Value delta_grad = 0;
        ad_trace("GE::my_backward 0");
        // todo: add ret value
        Value &source_grad = (Value &) source->grad;
        uint32_t size = source->size;

        ad_trace("GE::my_backward 1");
        if (source->size == 1 && target->size == 1 && !target->placeholder) {
            // Downgrade to scalar op
            ad_trace("GE::my_backward 2");
            delta_grad = source->accum(select(mask, target->grad, 0.f), 1);
        }

        ad_trace("GE::my_backward 3");
        if (!source_grad.valid())
            source_grad = zeros<Value>(size);
        else if ((uint32_t) source_grad.size() != size)
            source_grad.resize(size);

        ad_trace("GE::my_backward 4");
        if (!delta_grad.valid())
            delta_grad = zeros<Value>(size);
        else if ((uint32_t) delta_grad.size() != size)
            delta_grad.resize(size);
        ad_trace("GE::my_backward source_size=%d target_size=%d", size, target->size);

        ad_trace("GE::my_backward 5 delta=%d  %.19f %.19lf", delta_grad, delta_grad, delta_grad);

        ad_trace("GE::my_backward 5");
        MaskGuard guard(mask_stack);
        if (permute) {
            ad_trace("GE::my_backward 6");
            scatter(source_grad, target->grad, offset, mask);
            ad_trace("GE::my_backward 6b");
            delta_grad = target->grad; // ! only for equal size now
        }
        else {
            ad_trace("GE::my_backward 7");
            scatter_reduce(ReduceOp::Add, source_grad, target->grad, offset, mask);
            ad_trace("GE::my_backward 7b");
            delta_grad = target->grad; // ! only for equal size now
        }

        ad_trace("GE::my_backward 8 delta=%d  %.19f %.19lf", delta_grad, delta_grad, delta_grad);
        return delta_grad;
    }
...
template <typename Value> struct ScatterEdge : Special {
    ...
    Value my_backward(Variable *source, const Variable *target, uint32_t) const {
        MaskGuard guard(mask_stack);
        return source->accum(gather<Value>(target->grad, offset, mask),
                      (uint32_t) width(offset));
    }
...
Value ad_traverse(ADMode mode, uint32_t flags) {
    ad_log(Debug, "ad_traverse(): my_begin.");
    ...
    static Value my_ans = 0.f;
    static int ad_traverse_depth = 0;
    if (ad_traverse_depth == 0) {
        ad_log(Debug, "ad_traverse(): my_ans reset");
        my_ans = 0.f;
    }
    ad_log(Debug, "ad_traverse(): ad_traverse_depth ++");
    ad_traverse_depth ++;
    ...
    for (EdgeRef &er : todo) {
        ...
        ad_trace("ad_traverse(): current a%u grad = %d", v0i, v0->grad);
        ad_trace("ad_traverse(): current a%u grad = %d", v1i, v1->grad);
        ad_trace("ad_traverse(): processing edge a%u -> a%u ..", v0i, v1i);

        if (v1->next_bwd == 0) {
            ad_trace("(v1->next_bwd == 0)");
        }
        ...
        if (unlikely(edge.special)) {
            ...
            {
                MaskEdge<Value>* me = dynamic_cast<MaskEdge<Value>*>(edge.special);
                GatherEdge<Value>* ge = nullptr;
                ScatterEdge<Value>* se = nullptr;
                if constexpr (is_array_v<Value>) {
                    ge = dynamic_cast<GatherEdge<Value>*>(edge.special);
                    se = dynamic_cast<ScatterEdge<Value>*>(edge.special);
                }
                ...
                if (v1->next_bwd == 0) {
                    if (me != nullptr) {
                        ad_trace("special 1");
                        // my_ans += 
                        me->my_backward(v1, v0, flags);
                    }
                    else if (ge != nullptr) {
                        ad_trace("special 20");
                        if constexpr (is_array_v<Value>) {
                            ad_trace("special 2");
                            ad_trace("v1.size %d  v0.size %d", v1->size, v0->size);
                            // my_ans += 
                            ge->my_backward(v1, v0, flags); // !  
                        }
                    }
                    else if (se != nullptr) {
                        ad_trace("special 30");
            ...
            Value tmp = v1->mul_accum(v0->grad, edge.weight, v0->size); 
            ad_trace("mul_accum");

            if (v1->next_bwd == 0 && v1->size > 1) {
                ad_trace("my_ans += mul_accum (A)");
                ad_trace("v1.size %d  v0.size %d", v1->size, v0->size);
                my_ans += tmp; // !
            }
    ...
    ad_log(Debug, "ad_traverse(): my_end.");
    ...
    ad_log(Debug, "ad_traverse(): ad_traverse_depth --");
    ad_traverse_depth --;
    ad_log(Debug, "ad_traverse(): ad_traverse_depth = %d", ad_traverse_depth);
    return my_ans;
}
...
template DRJIT_EXPORT Value ad_traverse<Value>(ADMode, uint32_t);
