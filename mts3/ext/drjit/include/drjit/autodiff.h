...
template <typename Value> Value ad_traverse(ADMode mode, uint32_t flags);
...
struct DiffArray : ArrayBase<value_t<Type_>, is_mask_v<Type_>, DiffArray<Type_>> {
    ...
    static Type traverse_(ADMode mode, uint32_t flags) {
        DRJIT_MARK_USED(flags);
        if constexpr (IsEnabled)
            return detail::ad_traverse<Type>(mode, flags);
        return 0;
    }
...
#define DRJIT_DECLARE_EXTERN_TEMPLATE(T, Mask, Index)                          \
    ...
    extern template DRJIT_AD_EXPORT T ad_traverse<T>(ADMode, uint32_t);     \
